...
class EnlightDiffusion(nn.Module):
    ...
    def p_sample_loop(self, img_lr, encoder, histroEncoder, return_all_timesteps = False):
        ...
        if not self.config.on_diffusion_from_scratch:
            with torch.no_grad():
                img_c, feats_cond = encoder(img_lr)

            cond = on_cond_or_center_selector(img_lr, img_c, self.config.on_cond)

            center = on_cond_or_center_selector(img_lr, img_c, self.config.on_center)
            center = center_adjustment(center, img_lr, img_c, self.config.zeta_coeff, self.config.zeta_power, self.config.zeta, histroEncoder)

        else:
            cond = on_cond_selector(img_lr,  self.config.on_cond)

            center, feats_cond = encoder(img_lr) #must be [0,1] range
...
class LitDiffusion(pl.LightningModule):
    def __init__(self, diffusion_model, encoder, config, histroEncoder=None): 
        ...
        if not self.hparams.on_diffusion_from_scratch:
            self.encoder = encoder.eval()
            for param in self.encoder.parameters():
                param.requires_grad = False

            self.histroEncoder = histroEncoder.eval()
    ...
    def loss_fn(self, noise_pred, noise, x_start, t, img_lr, center):
        ...
        if self.hparams.loss_type == 'l1':
            ...
            loss = 0.9*loss_dfpl(x_start, t, center, noise_pred,
                                  noise)+0.1*F.smooth_l1_loss(noise_pred, noise)
        else:
    ...
    def training_step(self, batch, batch_idxs):
        ...
        if not self.hparams.on_diffusion_from_scratch:
            ...
            center = on_cond_or_center_selector(img_lr, img_c, self.hparams.on_center)
            center = center_adjustment(center, img_lr, img_c, self.hparams.zeta_coeff, self.hparams.zeta_power, self.hparams.zeta, self.histroEncoder)

        else:
            cond = on_cond_selector(img_lr,  self.hparams.on_cond)

            center, feats_cond = self.encoder(img_lr)
            options = ['pred', 'ref']
    ...
    def forward(self, img_path):
        ...
        from dataset import get_patch2, load_img
        from einops import rearrange
        ...
        if not self.hparams.paddingMode:
            img_lr, img_tar = get_patch2(img_in, img_in, -1, False)
        else:
            img_lr = img_in
