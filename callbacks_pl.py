...
class ColorMapLogger(pl.Callback):
    def __init__(self, val_samples, num_samples=9):
        super().__init__()
        self.img_lr, self.img_hr, self.img_lr_name = val_samples
        self.img_lr = self.img_lr[:num_samples]
        self.img_hr = self.img_hr[:num_samples] #cpu

        self.color_hr = color_map(self.img_hr)
        self.color_hr = make_grid(self.color_hr, nrow=3)

    def _share_step(self, img_lr, stage, trainer, pl_module):
        if trainer.strategy.is_global_zero:
            img_lr = img_lr.to(pl_module.device)
            
            color_pred, feats = pl_module(img_lr) 

            color_pred = make_grid(color_pred, nrow=3)
            # should be include all kind of image ? or just color map ? 
            c = color_map(img_lr)
            n = noise_map(c)
            h = histro_equalize(img_lr)

            n = make_grid(n, nrow=3)
            h = make_grid(h, nrow=3)
            c = make_grid(c, nrow=3)
            if isinstance(trainer.logger, WandbLogger):
                trainer.logger.experiment.log(
                    {
                    f"{stage}/output": [
                    wandb.Image(n, caption = 'noise_map'), #h,
                    wandb.Image(h, caption = 'HE'), #n
                    wandb.Image(c, caption = 'input'),
                    wandb.Image(color_pred, caption='pred'), #c_out
                    wandb.Image(self.color_hr, caption='ref'), #c_hr
                    wandb.Image(make_grid(self.img_hr, nrow=3), caption = 'ref_RGB')
                    ], 
                    f'{stage}/Current_epoch': trainer.current_epoch
                    }
                )
        trainer.strategy.barrier()

    def on_validation_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        self._share_step(self.img_lr, 'valid', trainer, pl_module)

    def on_test_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        self._share_step(self.img_lr, 'test', trainer, pl_module)

class HELogger(pl.Callback):
    def __init__(self, val_samples, num_samples=9):
        super().__init__()
        self.img_lr, self.img_hr, self.img_lr_name = val_samples
        self.img_lr = self.img_lr[:num_samples]
        ...
        self.he_hr = histro_equalize(self.img_hr)
        self.he_hr = make_grid(self.he_hr, nrow=3)

    def _share_step(self, img_lr, stage, trainer, pl_module):
        if trainer.strategy.is_global_zero:
            img_lr = img_lr.to(pl_module.device)

            he_pred, feats = pl_module(img_lr)

            he_pred = make_grid(he_pred, nrow=3)
            # should be include all kind of image ? or just color map ?
            h = histro_equalize(img_lr)
            ...
            if isinstance(trainer.logger, WandbLogger):
                trainer.logger.experiment.log(
                    {
                        f"{stage}/output": [
                            wandb.Image(h, caption='he_low'),  # n
                            wandb.Image(he_pred, caption='he_pred'),  # c_out
                            wandb.Image(self.he_hr, caption='he_ref'),  # c_hr
                            wandb.Image(
                                make_grid(self.img_hr, nrow=3), caption='ref')
                        ],
                        f'{stage}/Current_epoch': trainer.current_epoch
                    }
                )
